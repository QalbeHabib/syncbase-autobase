...
class SyncBase extends ReadyResource {
  ...
  async _apply(nodes, view, host) {
    await view.ready(); // Ensure the view is ready before applying any nodes
        ...
        try {
          if (pass && pass.base && this.crypto.publicKey) {
            ...
            const recordPublicKey = async () => {
              try {
                // Register the public key in the view for mutual authentication
                console.log(
                  "Registering our public key for authentication (mutual writer approach)"
                );
                // TODO: Define a proper schema for authorized_key if needed
                await pass.base.view.insert("@server/authorized_key", {
                  key: b4a.toString(this.crypto.publicKey, "hex"),
                  addedAt: Date.now(),
                });
                console.log("Successfully registered our public key");
              } catch (err) {
                console.error("Error registering public key:", err.message);
              }
            };
