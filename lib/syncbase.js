      ...
      try {
        ...
        try {
          if (node.value.type == 'claim-invite') {
            ...
            const user = {
              id: node.value.user.id,
              publicKey: node.value.user.id,
              username: "User",
              joinedAt: Date.now(),
              inviteCode: node.value.user.code,
              avatar: "1",
              status: "Chilling"
            }
            const role = {
              userId: node.value.user.id, updatedBy: node.value.user.id, updatedAt: Date.now(), role: "MEMBER"
            }

            await view.insert('@server/role', role)
            await view.insert('@server/user', user)
  ...
  async _setupReplication() {
    ...
    this.member = this.pairing.addMember({
      ...
      onadd: async (candidate) => {
        ...
        await this.base.append({ type: 'claim-invite', user: { id: b4a.toString(candidate.userData, 'hex'), code: b4a.toString(inv.id, 'hex') } }, { optimistic: true })
        candidate.confirm({
          key: this.base.key,
