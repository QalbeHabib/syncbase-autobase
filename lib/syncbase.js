...
class SyncBase extends ReadyResource {
  ...
  _initAutobase(options) {
    ...
    this.base = new Autobase(this.store, key, {
      encrypt: true,
      encryptionKey,
      optimistic: true, // Critical for mutual writer approach - allows non-writers to append
      valueEncoding: "json",
  ...
  static pair(store, inviteCode, options = {}) {
    console.log("Creating enhanced pairer");
...
class SyncBasePairer extends ReadyResource {
  ...
  _whenWritable() {
    ...
    setTimeout(() => {
      ...
      if (checkCount < maxChecks) {
        console.log(
          `Writable check (${checkCount + 1}/${maxChecks}): ${
            this.pass?.base?.writable
          }`
        );
        checkCount = maxChecks;
