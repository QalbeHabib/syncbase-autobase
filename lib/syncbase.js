...
class SyncBase extends ReadyResource {
    ...
    for (const node of nodes) {
      try {
        ...
        const isValid = await this.validator.validateAction(
          action,
          node.from.key,
          b4a.isBuffer(action.signer) ? action.signer : b4a.from(action.signer),
          view,
          true
        )
        ...
        if (!isValid) {
          ...
          continue
        } else host.ackWriter(node.from.key)
        ...
        const result = await this.router.dispatch(action, {
          view,
          base: this.base,
  ...
  async _parseAction(value) {
    ...
    try {
      // Directly parse the buffer as a JSON object
      const action = JSON.parse(value.toString())

      // Assign a default type if not present
      ...
      return {
        ...
        signer: action.signer ? b4a.from(action.signer) : null
      }
    } catch (err) {
