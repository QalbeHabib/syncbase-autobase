...
class SyncBase extends ReadyResource {
  ...
  _initAutobase(options) {
    ...
    this.base = new Autobase(this.store, key, {
      ...
      conflictResolution: async (a, b) => {
        // If one action is newer, prefer it
        if (a.timestamp && b.timestamp) {
          return a.timestamp > b.timestamp ? a : b
        }
        // If one has a higher sequence number, prefer it
        if (a.sequence && b.sequence) {
          return a.sequence > b.sequence ? a : b
        }
        // Default to first action if no clear resolution
        return a
      }
    })
  ...
  async _apply(nodes, view, host) {
    ...
    for (const node of nodes) {
      try {
        ...
        const nodeKey = node.key ? node.key.toString('hex') :
        ...
        // If it's a buffer and we can't parse it, skip it
        if (node.value && !node.type && !node.payload) {
          try {
            // Direct use of the router's dispatcher with the raw buffer
            const result = await this.router.router.dispatch(node.value, {
              view,
              base: this.base,
            ...
            // If direct dispatch fails, try to parse the action
            console.log('Direct dispatch failed, trying to parse action:', err.message);
          }
        }
        ...
        const { dispatch } = require('./components/spec/hyperdispatch');
        try {
          // Use hyperdispatch to directly process the message
          const encodedMessage = dispatch(action.type, action.payload);
          const result = await this.router.router.dispatch(encodedMessage, {
            view,
            base: this.base,
            authorKey: node.from?.key
          });
  ...
  async _setupReplication() {
    ...
    this.swarm = new Hyperswarm({
      ...
      rateLimit: this.replicationRateLimit
    })
    ...
    this.swarm.on('connection', (connection, peerInfo) => {
      // Check rate limiting
      const now = Date.now()
      if (now - this.lastReplicationTime < this.replicationRateLimit) {
        connection.destroy()
        return
      }
