  ...
  async _apply(nodes, view, host) {
    for (const node of nodes) {
      try {
        ...
        // Log details before validation
        console.log('Validation Details:', {
          payload: action.payload,
          signature: action.signature ? action.signature.toString('hex') : 'No signature',
          signer: action.signer ? action.signer.toString('hex') : 'No signer',
          fromKey: node.from?.key ? node.from.key.toString('hex') : 'No from key'
        })
        // Validate the action
        ...
        const result = this.router.dispatch(action.type, action.payload, {
          view,
          base: this.base,
