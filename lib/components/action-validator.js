...
class ActionValidator {
  ...
  static ACTION_TYPE_MAP = {
    'INITIALIZE_SERVER': '@server/create-server',
  ...
  async validateAction(payload, signature, authorKey, signer, view, optimistic, action) {
    ...
    if (!this._verifySignature(payload, signature, signer)) {
      console.log('Invalid signature')
      return false
    }

    // Determine the route name
    const routeName = ActionValidator.ACTION_TYPE_MAP[action.type] || action.type
    console.log('Validating action:', { routeName, action })

    // Use the route name for validation
    switch (routeName) {
      case '@server/create-server':
        return this._validateCreateServer(action, authorKey, view)
      case '@server/create-channel':
        return this._validateCreateChannel(action, authorKey, view)
      case '@server/send-message':
        return this._validateSendMessage(action, authorKey, view)
      case '@server/set-role':
        return this._validateSetRole(action, authorKey, view)
      case '@server/create-invite':
        return this._validateCreateInvite(action, authorKey, view)
      case '@server/claim-invite':
        return this._validateClaimInvite(action, authorKey, view)
      case '@server/delete-message':
        return this._validateDeleteMessage(action, authorKey, view)
      case '@server/edit-message':
        return this._validateEditMessage(action, authorKey, view)
      case '@server/update-server':
        return this._validateUpdateServer(action, authorKey, view)
      case '@server/update-channel':
        return this._validateUpdateChannel(action, authorKey, view)
      case '@server/delete-channel':
        return this._validateDeleteChannel(action, authorKey, view)
      default:
  ...
  async _validateCreateServer(action, authorKey, view) {
    try {
      ...
      console.log('Create server validation:', { isValid, payload })
      return isValid
    } catch (err) {
  ...
  async _validateClaimInvite(action, authorKey, view) {
    ...
    const authorId = b5a.toString(authorKey, 'hex')
