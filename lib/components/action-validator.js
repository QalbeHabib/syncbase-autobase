...
class ActionValidator {
  ...
  async getUserRole(action, view) {
    const signerBuffer = b4a.isBuffer(action.signer) ? action.signer : b4a.from(action.signer)
   ...
   */
  async validateAction(payload, signature, authorKey, signer, view, optimistic, action) {
    try {
      // Convert authorKey to hex string
      let authorId = authorKey ? b4a.toString(authorKey, 'hex') : null

      // Verify the signature first
      const signatureIsValid = optimistic || this._verifySignature(payload, signature, signer)
  ...
  async _validateCreateServer(action, authorKey, view) {
    try {
      ...
      const isValid = !!(
        payload.id &&
        payload.name &&
        (payload.createdAt || true) // createdAt is optional
      )
  ...
  async _validateCreateChannel(action, authorId, view) {
    ...
    const existingChannelById = await this.syncbase.channels.getChannel(action.payload.channelId)

    if (existingChannelById?.channelId == action.payload.channelId) {
    ...
    if (existingChannelByName?.name == action.payload.name) {
      return false
    }
  ...
  async _validateUpdateChannel(action, authorId, view) {
    // Check if the channel exists
    const channel = await this.syncbase.channels.getChannel(action.payload.channelId)
    if (!channel) {
      return false
    }
    ...
    if (action.payload.name && action.payload.name !== channel.name) {
      ...
      if (existingChannelByName.name == action.payload.name && existingChannelByName.channelId !== action.payload.channelId) {
        return false
      }
    }
  ...
  async _validateDeleteMessage(action, authorId, view) {
    ...
    const signerPubKey = b4a.isBuffer(action.signer) ? action.signer : b4a.from(action.signer)
  ...
  async _validateEditMessage(action, authorId, view) {
    // Check if the message exists
    const message = await this.syncbase.messages.getMessage(action.payload.id, action.payload.channelId)
    if (!message) {
      return false
    }
  ...
  async _validateRevokeInvite(action, authorId, view) {
    ...
    const invite = await view.findOne('@server/invite', { code: action.payload.code })
    if (!invite) {
      return false
    }
  ...
  _hasPermission(role, permission) {
    const rolePermissions = {
      'OWNER': [
        ...
        'CREATE_INVITES', 'EDIT_SERVER', 'EDIT_CHANNEL', 'DELETE_CHANNEL', 'MANAGE_INVITES'
      ],
      'ADMIN': [
        'MANAGE_CHANNELS', 'SEND_MESSAGES', 'DELETE_MESSAGES', 'SET_ROLE',
        'CREATE_INVITES', 'EDIT_CHANNEL', 'MANAGE_SERVER', 'MANAGE_INVITES'
      ],
      'MODERATOR': [
        'SEND_MESSAGES', 'DELETE_MESSAGES', 'CREATE_INVITES', 'MANAGE_INVITES'
      ],
      'MEMBER': [
        'SEND_MESSAGES'
      ]
