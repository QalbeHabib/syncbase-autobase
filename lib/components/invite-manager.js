...
class InviteManager {
  ...
  async createInvite(options = {}, retryCount = 0) {
    // Check permission
    try {
      await this.syncBase.ready();
      await this.syncBase.base.view.ready(); // Ensure view is ready
      ...
      for (let i = 0; i < 3; i++) {
        try {
          ...
          console.warn(
            `Error in update ${i + 1} after invite creation: ${updateErr.message
            }`
          );
        }
      }
      ...
      // Verify the invite was stored and add the inviteCode string
      const storedInviteCheck = await this.syncBase.base.view.findOne("@server/invite", {
        id: id,
      });
      const inviteIdHex = b4a.toString(id, "hex");

      if (storedInviteCheck) {
        console.log(
          `Successfully verified invite in view with ID: ${inviteIdHex}`
        );

        // Now update the record to include the inviteCode by re-inserting with the same key
        try {
          await this.syncBase.base.view.ready(); // Ensure view is ready
          await this.syncBase.base.view.insert("@server/invite", {
            // Use the data from the initial insert attempt, plus the code
            id: id,
            invite: invite,
            publicKey: publicKey,
            expires: expires,
            uses: 0,        // Make sure all fields are included
            maxUses: maxUses,
            timestamp: extendedPayload.timestamp,
            uniqueId: extendedPayload.uniqueId,
            serverId: extendedPayload.serverId,
            createdBy: extendedPayload.createdBy,
            expiresAt: extendedPayload.expiresAt,
            // Add the new field
            inviteCode: inviteCode,
          });
          await this.syncBase.base.view.flush(); // Ensure update (via insert) is processed
          console.log(`Successfully updated invite ${inviteIdHex} with inviteCode.`);
        } catch (updateErr) {
          console.error(
            `Error updating invite record with code: ${updateErr.message}`
          );
          // Log critical failure if update doesn't work
        }
      } else {
        console.warn(
          `Invite with ID ${inviteIdHex} not found in view after creation attempt`
        );
        // If the initial insert failed, throw an error
        throw new Error("Failed to store the initial invite record.");
      }
