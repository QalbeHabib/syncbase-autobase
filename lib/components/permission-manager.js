...
class PermissionManager {
  ...
  async setRole({ userId, serverId, role, timestamp = Date.now() }) {
    ...
    const action = this.syncBase.crypto.createSignedAction("@server/set-role", {
      ...
      role,
      updatedAt: Date.now(),
      updatedBy: this.syncBase.crypto.publicKey,
    });
  ...
  async getUserRole(userId, serverId) {
    const roleRecord = await this.syncBase.base.view.findOne("@server/role", {
      userId,
      serverId,
    });
  ...
  async getUsersWithRole(serverId, role) {
    const roleRecords = await this.syncBase.base.view.find("@server/role", {
      serverId,
      role,
    });
    ...
    for (const record of roleRecords) {
      const user = await this.syncBase.base.view.findOne("@server/user", {
        id: record.userId,
      });
      if (user) {
        users.push(user);
  ...
  async getServerOwner(serverId) {
    ...
    return this.syncBase.base.view.findOne("@server/user", {
      id: ownerRole.userId,
    });
  }
