...
class MessageManager {
  ...
  async sendMessage({ channelId, content, attachments = [] }) {
    try {
      ...
      if (await !this.syncBase.hasPermission('SEND_MESSAGE')) {
        return false
      }
  ...
  async editMessage({ messageId, content }) {
    try {
      ...
      const message = await this.syncBase.base.view.get('@server/message', { id: messageId })
      if (!message) {
  ...
  async getMessage(messageId) {
    return this.syncBase.base.view.get('@server/message', { id: messageId })
  }
