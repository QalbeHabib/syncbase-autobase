...
class MessageManager {
  ...
  async sendMessage({ channelId, content, attachments = [] }) {
    ...
    const action = this.syncBase.crypto.createSignedAction('@server/send-message', {
      id,
      channelId,
      content,
      attachments,
  ...
  async editMessage({ messageId, content }) {
    ...
    const action = this.syncBase.crypto.createSignedAction('@server/edit-message', {
      id: messageId,
      content,
      timestamp: Date.now()
    })
  ...
  async deleteMessage({ messageId }) {
    ...
    const action = this.syncBase.crypto.createSignedAction('@server/delete-message', {
      id: messageId,
      timestamp: Date.now()
    })
