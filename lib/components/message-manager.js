...
class MessageManager {
  ...
  async sendMessage({ channelId, content, attachments = [] }) {
    try {
      ...
      const channel = await this.syncBase.base.view.findOne('@server/channel', { channelId: channelId })
      if (!channel) {
  ...
  async getMessages(query) {
    ...
    const stream = await this.syncBase.base.view.find('@server/message', query || {})
    const messages = []
