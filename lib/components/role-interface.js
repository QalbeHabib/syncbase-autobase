...
class RoleInterface {
  ...
  async assignRole(userId, roleId, options = {}) {
    try {
      return await this.syncBase.roleManager.assignRole(userId, roleId, options)
    } catch (err) {
  ...
  async removeRole(userId, roleId, options = {}) {
    try {
      return await this.syncBase.roleManager.removeRole(userId, roleId, options)
    } catch (err) {
  ...
  async listBannedUsers(options = {}) {
    try {
      ...
      for (const [key, banInfo] of this.syncBase._bannedUsers.entries()) {
        ...
        } else if (options.channelId && banInfo.channelId === options.channelId) {
