...
class ChannelManager {
  ...
  async createChannel({
    ...
    try {
      ...
      if (!serverInfo || !serverInfo.id) {
        throw new Error(
          "Cannot verify server identity - unauthorized channel creation rejected"
        );
      }
      ...
      const hasChannelPermission =
        await this.syncBase.permissions.hasPermission("CREATE_CHANNEL");
      const hasAdminPermission = await this.syncBase.permissions.hasPermission(
        "ADMINISTRATOR"
      );
      ...
      if (hasChannelPermission || hasAdminPermission) {
        ...
        const isAdmin =
          await this.syncBase.serverInitializer.hasAdminPermission();
      ...
      if (!isAuthorized) {
        throw new Error(
          "Unauthorized: You do not have permission to create channels in this server"
        );
      }
  ...
  async getChannel(channelId) {
    const channel = await this.syncBase.base.view.get("@server/channel", {
      channelId: channelId,
    });
  ...
  async getChannelsByType(type) {
    ...
    const allChannels = await this.syncBase.base.view.find("@server/channel", {
      type,
    });
  ...
  async findChannelByName(name) {
    ...
    return this.syncBase.base.view.findOne("@server/channel", {
      serverId,
      name,
    });
  }
