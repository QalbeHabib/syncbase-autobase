...
class ServerInitializer {
  ...
  async initialize({
    ...
    try {
      ...
      await this.syncBase.base.append(
        this.syncBase.router.router.dispatch('@server/create-server', serverInitAction),
        { optimistic: true }
      )
      ...
      // Create user record
      const userAction = this.syncBase.crypto.createSignedAction('@server/claim-invite', {
        id: currentUserId,
      ...
      await this.syncBase.base.append(
        this.syncBase.router.router.dispatch('@server/claim-invite', userAction),
        { optimistic: true }
      )
      ...
      await this.syncBase.base.append(
        this.syncBase.router.router.dispatch('@server/create-channel', generalChannelAction),
        { optimistic: true }
      )
  ...
  async updateServerInfo({ name, description }) {
    ...
    // Create action
    const action = this.syncBase.crypto.createSignedAction('@server/update-server', updatedInfo)

    // Dispatch the action
    await this.syncBase.base.append(this.syncBase.router.router.dispatch('@server/update-server', action), { optimistic: true })
