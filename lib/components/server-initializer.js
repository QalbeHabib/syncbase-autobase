...
class ServerInitializer {
  ...
  async isInitialized() {
    try {
      const server = await this.syncBase.base.view.findOne(
        "@server/server",
        {}
      );
      return !!server;
   ...
   */
  async initialize({ name = "My SyncBase Server", description = "" }) {
    try {
      ...
      const serverInitAction = this.syncBase.crypto.createSignedAction(
  ...
  async getServerInfo() {
    try {
      ...
      const serverInfo = await this.syncBase.base.view.findOne(
        "@server/server",
        {}
      );
  ...
  async hasAdminPermission() {
    try {
      ...
      return (
        userRole && (userRole.role === "OWNER" || userRole.role === "ADMIN")
      );
    } catch (err) {
  ...
  async updateServerInfo({ name, description }) {
    ...
    try {
      ...
      if (!(await this.hasAdminPermission())) {
        throw new Error(
          "You do not have permission to update server information"
        );
      }
      ...
      const updatedInfo = {
        ...
        description:
          description !== undefined ? description : serverInfo.description,
        createdBy: serverInfo.createdBy,
        createdAt: serverInfo.createdAt,
      };

      const updateAction = this.syncBase.crypto.createSignedAction(
