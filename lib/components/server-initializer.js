...
class ServerInitializer {
   ...
   */
  async initialize({ name, description = '' }) {
    try {
      ...
      const actionJson = JSON.stringify(initializeAction)

      // Create a buffer with type ID 1 (INITIALIZE_SERVER) at the beginning
      const actionBuffer = Buffer.concat([
        Buffer.from([1]), // Type ID 1 for INITIALIZE_SERVER
        Buffer.from(actionJson)
      ])

      // Append the raw action buffer directly
      console.log('Appending raw server initialization action')
      await this.syncBase.base.append(actionBuffer)
  ...
  async updateServerInfo({ name, description }) {
    ...
    // Dispatch the action
    await this.syncBase.base.append(dispatch('@server/update-server', action))
