  ...
  constructor(syncBase, validator) {
    ...
    this.router.add("@server/create-invite", async (data, context) => {
      ...
      try {
        ...
        if (!data || !data.id || !data.invite || !data.publicKey) {
          console.error("Invalid invite data:", data);
          return false;
        }

        console.log(`Creating invite with ID: ${b4a.toString(data.id, "hex")}`);

        // First check if this invite already exists
        try {
          ...
          if (existingInvite) {
            console.warn(
              `Invite with ID ${b4a.toString(data.id, "hex")} already exists`
            );
            return false;
          }
        } catch (lookupErr) {
          // Ignore lookup errors and continue with insert
          console.log("Error checking for existing invite:", lookupErr.message);
        }

        // Insert the invite and flush the view
        await view.insert("@server/invite", data);
        await view.flush();

        console.log(
          `Successfully created invite with ID: ${b4a.toString(data.id, "hex")}`
        );
        return true;
      } catch (error) {
    ...
    this.router.add("@server/claim-invite", async (data, context) => {
      ...
      try {
        ...
        if (!data.userId && authorKey) {
          console.log("Using authorKey as userId since data.userId is missing");
          data.userId = b4a.toString(authorKey, "hex");
        }

        if (!data.userId) {
          console.error("Cannot claim invite: Missing userId");
          return false;
        }

        if (!data.publicKey && authorKey) {
          console.log(
            "Using authorKey as publicKey since data.publicKey is missing"
          );
          data.publicKey = authorKey;
        }
        ...
        try {
          ...
          const existingUser = await view.findOne("@server/user", {
            id: data.userId,
          });
          if (existingUser) {
            console.log(
              `User ${data.userId} already exists, skipping user creation`
            );
          } else {
            ...
            console.log(`Added user ${data.userId}`);
          }

          // Check if role already exists
          const existingRole = await view.findOne("@server/role", {
            userId: data.userId,
          });
          if (existingRole) {
            console.log(
              `Role for user ${data.userId} already exists, skipping role creation`
            );
          } else {
            await view.insert("@server/role", {
              userId: data.userId,
            ...
            console.log(`Added member role for user ${data.userId}`);
          }
          ...
          console.error("Database error in claim invite:", dbError.message);
          return false;
        }
      } catch (error) {
