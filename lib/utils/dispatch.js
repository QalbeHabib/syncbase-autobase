...
function dispatch(type, action) {
  ...
  // Ensure the action includes its signature
  if (!action.signature) {
    throw new Error('Action must be signed before dispatching')
  }

  // Create JSON representation of the entire signed action
  const actionBuffer = b4a.from(JSON.stringify(action))

  // Combine the buffers
  return b4a.concat([buffer, actionBuffer])
}
